<?php 
    ...
    if (isset($_POST['upd_contacts']))
    {
        ...
        $q = "UPDATE `contact_details` SET `address`=?,`gmap`=?,`pn1`=?,`pn2`=?,`email`=?,`fb`=?,`insta`=?,`tw`=?,`iframe`=? WHERE `s_no`=?";
                ...
                <div class="col-md-2 mb-3">
                    <div class="card bg-dark text-white">
                        ...
                        <div class="card-img-overlay text-end">
                            <button type="button" onclick="rem_member($row[s_no])" class="btn btn-danger btn-sm shadow-none">
    ...
    if (isset($_POST['rem_member']))
    {
        ...
        $pre_q = "SELECT * FROM `team_details` WHERE `s_no`=?";
        $res = select($pre_q,$values,'i');
        $img = mysqli_fetch_assoc($res);

        if(deleteImage($img['picture'],ABOUT_FOLDER)){
            $q = "DELETE FROM `team_details` WHERE `s_no`=?";
